...
class AlchemyBaseMProtoTagger(AlchemyModel):
    ...
    def __init__(self):
        ...
        self.tagging_scheme = IOTaggingScheme(task.entity_types)
    ...
    def sinkhorn_matching_and_ema(
        ...
        for k in range(self.tagging_scheme.num_tags):
            ...
            if k == 0:
                # 只分析unlabeled entities，这些东西仅包含在O中
                if sym_tbl().contains_global("assignments"):
                    sym_tbl().get_global("assignments").append(assignment.clone().detach().cpu())
                if sym_tbl().contains_global("similarity"):
                    # n_o, k
                    sym_tbl().get_global("similarity").append(torch.max(logits[mask_k], dim=-1).values.detach().cpu())
    ...
    def sinkhorn_matching_and_ema(
        ...
        for k in range(self.tagging_scheme.num_tags):
            ...
            if k == 0:
                ...
                onehot_indexes = onehot_indexes[..., :self.num_proto_per_type]
            else:
                # 依据匹配选定target
                well_pred_mask_k = well_pred_mask[mask_k]       # gt为k的token，是否预测正确
